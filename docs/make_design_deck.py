...
STRINGS = {
    "ko": {
        ...
        "arch_sem": "의미 검색 (확장)",
        "arch_sem_body": "HF 임베딩 모델(기본 bge-m3) → PostgreSQL + pgvector (law_chunks)",
        "arch_out": "터미널 출력",
        ...
        "sem_title": "의미 검색 확장 — CLI 설계",
        "sem_sub": 'law-cli --semantic "질의문" [--model HF모델] [--law-filter 키워드] [--top-k N] [--db 이름]',
        "sem_bullets": [
            (0, "흐름", True),
            (1, "대상 수집(--law-filter·--type) → 파일 해시 비교로 증분 동기화(변경분만 임베딩)", False),
            (1, "질의문 임베딩 → pgvector cosine top-k → 미리보기·출처·재조회 명령 출력", False),
        ...
        "ops_bullets": [
            ...
            (0, "optional extra semantic: sentence-transformers, psycopg[binary]", False),
        ...
        "roadmap_title": "로드맵",
        "roadmap_bullets": [
            (0, "1) CLI 연결 마무리: --semantic 옵션군을 argparse에 연결 + 단위 테스트", False),
            (0, "2) 통합 테스트: 로컬 PostgreSQL 대상 스모크 테스트 (있을 때만 실행)", False),
            (0, "3) README 갱신: 의미 검색 사용법·DB 준비 절차", False),
